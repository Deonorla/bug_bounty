...
const Sidebar = () => {
  ...
  const menus = [
    { name: "Home", link: "", icon: MdDashboard },
    {
      name: "Bounties",
      link: "",
      ...
      <div className="mt-8 flex flex-col gap-4 relative">
        {menus?.map((menu, i) => (
          <div key={i} className="group">
            <div
              className="flex items-center  cursor-pointer text-sm gap-3.5 font-medium p-2 mx-2 hover:bg-white/10 rounded-md"
              onClick={() => {
                !menu.subMenu
                  ? navigate(`${menu.link}`)
            ...
            >
              ...
              <h3
                ...
                className={` ${
                  open && "hidden "
                } absolute z-50 bg-white/10 whitespace-pre text-[#ffffff], rounded-md drop-shadow-lg px-0 py-0 w-0 overflow-hidden group-hover:px-2 group-hover:py-1 group-hover:left-[5rem] group-hover:duration-300 group-hover:w-fit `}
              >
                {menu.name}
              </h3>
              {/* {menu.subMenu && (
