...
export const GlareCard = ({
  ...
  const backgroundStyle = {
    ...
    "--rainbow":
      "repeating-linear-gradient( 0deg, #F9BF2D calc(var(--step) * 1), #FF5E4A calc(var(--step) * 2), #F9BF2D calc(var(--step) * 3), #FF5E4A calc(var(--step) * 4), #F9BF2D calc(var(--step) * 5), #FF5E4A calc(var(--step) * 6), #F9BF2D calc(var(--step) * 7) ) 0% var(--bg-y)/200% 700% no-repeat",
    "--diagonal":
      "repeating-linear-gradient( 128deg, #0e152e 0%, #F9BF2D 3.8%, #F9BF2D 4.5%, #F9BF2D 5.2%, #0e152e 10%, #0e152e 12% ) var(--bg-x) var(--bg-y)/300% no-repeat",
    "--shade":
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div
            className={cn(
              "h-full w-full bg-gradient-to-r from-linear-1  to-linear-2  text-[transparent] bg-clip-text",
              className
            )}
          >
            {children}
