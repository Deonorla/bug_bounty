...
const BugModel = (props: any) => {
  const { scene, animations } = useGLTF(`/des.glb`);
  const mixer = useRef<THREE.AnimationMixer | undefined>(undefined); // Define mixer as a RefObject

  // Ensure mixer is initialized when animations are loaded
  if (animations && animations.length && !mixer.current) {
    mixer.current = new THREE.AnimationMixer(scene);
    animations.forEach((clip) => {
      mixer.current!.clipAction(clip).play(); // Assert mixer.current is not null or undefined
    });
  }
